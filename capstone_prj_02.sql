...
FROM
    albums
ORDER BY release_year ASC limit 2 -- 1988
...
alter table albums 
rename column name to name_a;

alter table bands 
rename column id to id_b;

SELECT 
    name_b AS band_name, name_a AS album_name
FROM
    bands
        INNER JOIN
    albums ON bands.id_b = albums.band_id
 
    
    -- Ex 4--
SELECT 
    name_b AS band_name, name AS album_name
FROM
    bands
        LEFT JOIN
    albums ON bands.id_b = albums.band_id        -- dream theatre has no album


     -- Ex 5--
     select name_a as album_name , count(name) as song_name , sum(length)  from 
     songs inner join albums on albums.id= songs.album_id  group by name_a;       --  death magnetic is the longest album.
     
    
    
     -- Ex 6 -- 
     INSERT INTO bands(id_b,name_b) 
    VALUES (8,'coldplay');
     
     
     INSERT INTO albums(id,name_a ,release_year,band_id) 
     values
     (19, 'hymn of the weekend', 2015, 8);
    ...
    delete from bands where id_b=8;
...
GROUP BY name
    ...
    -- Ex 9 --
SELECT 
    name_a AS album_name, name AS songs, length
FROM
    songs
        INNER JOIN
...
SELECT 
    name_b AS band_name, COUNT(name_a) AS no_of_albums
FROM
    albums
        INNER JOIN
    bands ON albums.band_id = bands.id_b
GROUP BY name_b

SELECT 
    name_a AS album_name, COUNT(name) AS no_of_songs
FROM
    songs
        INNER JOIN
    albums ON songs.album_id = albums.id
